...
import { FormControl, FormControlLabel, Radio, RadioGroup, Box, Typography, Stack, Button, ButtonBase, Divider } from '@mui/material'
      ...
      <FormControl>
        ...
        >
          ...
          <FormControlLabel value={false} control={<Radio />} label={<Typography variant='body2' fontSize={14.5}>Select Random Backgrounds from unsplash.com</Typography>} />
            <Box pr={1} pl={3} mt={.5} sx={{position: "relative", opacity: 1}}>
              {/* <Box sx={{position: "absolute", top: 0, left: 0, height: "100%", width: "100%", zIndex: 1, background: "rgba(0,0,0,.5)"}}></Box> */}
              <Box sx={{display: "flex", gap: .25, overflowX: "scroll", overflowY: "hidden", width: 390, mb: 1}} component="ul">
                {localBackgrounds.map(item => (
                  <ButtonBase 
                  ...
                  >
                    <Image src={item.thumbnailUrl} width={50} duration={0}/>
                  </ButtonBase>
                ))}
              </Box>
              <Box sx={{display: "flex", justifyContent: "right"}}>
